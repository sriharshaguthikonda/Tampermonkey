...
| inventory ref | feature | current selector/logic | class | target pack anchor | sub-stage | reason |
|---|---|---|---|---|---|---|
| `35-server-tts.js:372` | 11 | `.group\/conversation-turn, [data-message-author-role="user"]` (event.target.closest) | route-via-driftwatch | userUnit | S3.9 — done (S3c) | dead Tailwind class + dead attr; hit-test moves to "userUnit containing the event target", exchanges enumerated (R6/S3.9) |
| `15-playback-lock.js:297` | 11 | `.group\/conversation-turn, .group\/turn-messages, [data-message-author-role]` (bus subscribe) | route-via-driftwatch | userUnit / exchangeRoot | S3.9 — done (S3c) | dead selectors; listener attachment migrates to per-`userUnit` under `exchangeRoot` (R6 finding 10) |
| `35-server-tts.js:374` | 11 | `button[aria-label="Edit message"]` | route-via-driftwatch | editMessageButton | S3.9 — done (S3c) | exact 1:1 match to pack-v2 `editMessageButton` (`edit.aria-exact`, `inside:userUnit`) — the R7 "Edit button" named example |
