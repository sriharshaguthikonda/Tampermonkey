...
## Round 2 answers (received via bridge, 2026-07-10)

**3.1 — RESOLVED, boundary accepted.** Reviewer: "exact-only" meant *remote-updatable but restricted to exact deterministic matchers*, not code-frozen. Accepted our "pack proposes candidates, code decides trust" with additions we adopt:
- Exact vocabulary confirmed: attr-equality, exact role, exact/oneOf accessible names, exact multi-attr combos. Banned: substring/regex names, text matching, heuristics, positional indices, broad structural traversal, generated CSS classes without stable co-evidence.
- **Positive intent checks, not just exclusions** (a pack could exactly name "Attach files"): action anchors act only through capability methods (`composer.submit()`, never generic `clickAnchor()`); code owns the submit-control relationship (button belongs to the same trusted composer instance as the resolved input); composer state must be "ready to submit"; expected prompt content present; all invariants rerun on freshly resolved nodes immediately before the click.
- **State-aware preflight** states: `valid_present / valid_expected_absent / invalid_candidate / ambiguous / unexpected_absent`; activation fails only on the last three where the tested state expects the anchor (Stop absent while idle ≠ rejection).
- Rollout asymmetry: read-anchor pack changes auto-activate after preflight; **action-anchor changes require explicit local approval in early versions** (accepted — cheap, no key management; supersedes the signing debate for v1, full signing stays a later hardening item).

**3.2 — RESOLVED.** Role+name stays first-class but names are *versioned, locale-scoped pack data*, not universal identifiers. Adopted:
- (a) NO positional fallback for action anchors ("last button in composer" is locale-neutral but not *intent*-neutral). Locale-neutral evidence = native/form semantics instead: `type="submit"`, exact testid, `button.form === composerInput.form`, unique submit-capable control in trusted form — plus a small code-implemented semantic-relation vocabulary (e.g. `associated-form-submit-control`), never arbitrary CSS ancestry/indexes. Positional strategies risk-gated: read=allowed (rooted+validated), navigate=cautious, input/action/destructive=prohibited.
- (b) Locale-explicit schema: `accessibleName.byLocale` groups or per-strategy `when.uiLocale`; adapter determines locale from code-owned signals (`<html lang>`, app locale, stored setting) — never from the candidate's own label (circular). `localeSensitivity` / `verifiedLocales` / `unknownLocalePolicy: "skip-strategy"`; unsupported locale → skip name strategy → exact locale-independent strategy → else fail closed for action anchors. Only observed+fixture-verified translations in packs; no machine-translation dictionaries. "Exact" = exact after narrow canonicalisation (Unicode norm, whitespace collapse, optional configured case-fold; never punctuation-stripping — "Send" ≠ "Send later").
- Authoring ranking revised from "attrs always first" to *strongest exact intent-bearing evidence*: meaningful testid → native/form semantics → role + locale-scoped name → exact attr combos → code-owned relations → structural (low-risk anchors only) → fuzzy (diagnostics/repair only). A random generated ID is worse than exact role+name; generic `type="button"` is weaker than role+name.

**3.3 — RESOLVED, envelope upgraded.** Self-test = "passive diagnostic resolver: no clicks/submits/focus/mutation ever". Adopted:
- Triggers: after stable adapter init + on meaningful SPA route transition + when a not-yet-observed state class appears + after pack activation + on resume after long background. NOT once-per-session. State classes declared per anchor (`applicableWhen: {conversation: "streaming"}`); session coverage set + `probeKey = pack:route:state:viewport` dedup.
- Budget: 4–8ms cancellable idle slices (rIC with timeout fallback), cumulative ~50ms cap, cancel on user input/navigation/streaming; probe resolves FRESH in diagnostic mode (never reuses production caches).
- Absence taxonomy fix (the key correction): `verified_present / verified_expected_absent / unobserved_applicable_state / unexpected_absent` (+ ambiguous, candidate_failed_invariants, probe_interrupted, unsupported_locale). *Absence in an inapplicable state proves the state model works — not that the anchor still resolves.* Health panel separates current health vs state coverage vs last positive verification vs pack-version coverage; stop-button gets probed opportunistically the next time the user naturally streams. Compact local coverage history persisted (result codes + strategy id + coarse date only — no DOM/URLs/titles). Fixture-verified labelled separately from live-verified, never merged into one green.
- Aggregation: Tier 1 default = local-only history + notify only on meaningful transitions (healthy→failed, primary→fallback, unique→ambiguous, verified→stale, pack rejected). Tier 2 (only if ever needed) = explicit opt-in FAILURE-ONLY aggregate reports: minimized payload (site/pack/anchor/state/result/day-bucket), no persistent identifier, local dedup one-per-anchor-result-pack-day, server aggregates+discards fast, open schema, user payload preview, alert threshold ≥5 reports crossing baseline. No DP/OHTTP machinery — premature.

**3.4 — RESOLVED, parallel spike accepted** under 5 conditions (all adopted): separate experimental module + branch; D/E never depend on spike success; no backend concepts (e.g. stream-complete) encoded into pack schema prematurely; spike ends in explicit **GO / LIMITED-GO / NO-GO per consumer** (export may be GO while TTS-speech is NO-GO); raw captures never become fixtures without allowlist sanitisation.

Spike design settled:
- **(d) Equivalence protocol** — no fuzzy similarity score. Two independently derived models (RawTurn from traffic/bootstrap only; RenderedTurn from DOM only — no cross-consultation), turn alignment by id > branch+role > sequence > text-assisted (with recorded confidence), then per-dimension gates: turn-set precision/recall (hard gate: ZERO hidden/system/internal turns exposed as visible — one leak fails the spike for TTS/export); text fidelity via narrow canonicalisation with every diff *classified* (presentation_whitespace vs content_missing vs hidden_content_exposed ...); structured content compared separately (code blocks, tables, citations, attachments); completion-event correctness vs a DOM ground-truth *window* (multi-signal: indicator gone + actions appear + quiet interval). Numeric gates: 0 premature completions, 0 duplicates, 100% visible completed-turn recall in core scenarios, p95 completion latency ≤~1s, interrupted/error classification 100%. Scenario corpus: ordinary/long-stream/code/citations/stop/error/regenerate/edit-branch/switch-during-generation/multi-tool/refresh-into-conversation. Failed comparisons emit reviewable classified diffs, not scores.
- **(e) Initial-load capture** — layered passive ingestion, blindness not accepted upfront: (1) document_start MAIN-world fetch wrap, recording `installedAtReadyState` (never assume the manifest guaranteed earliness); (2) bounded polling (≤2s) of the known bootstrap global (`__reactRouterContext.state.loaderData`) with a **versioned allowlist projector run in MAIN world before anything crosses the bridge** — the bootstrap object contains access tokens/email/identity, so raw loader state is a SECRET (no setter-traps on the global, no patching JSON.parse/React internals); (3) DOM reconstruction fallback; (4) explicit `BootstrapCoverage` taxonomy surfaced in health panel. Field-level source precedence: identity/graph/raw content = network/bootstrap; visible text/presentation = rendered DOM; completion = corroborated by both. Capture pipeline: `captures/raw/` (git-ignored, local, treated as secret) → allowlist projector (fails if unknown fields survive) → `fixtures/transcripts/` (committed). Reviewer's own predicted best outcome adopted as the spike's hybrid hypothesis: *network owns identity/graph/metadata, DOM owns presentation, completion requires corroboration.*

**Round 2 verdict: all four contested items resolved. No open disputes. Plan v2 rewrite can proceed.**
