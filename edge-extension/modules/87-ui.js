(function () {
    ...
    // Overlay "Prompt" group (ChatGPT only): [id suffix, CONFIG key, setter, profile storage key, label].
    const PROMPT_TOGGLES = [
        ['enter-to-send', 'ENTER_TO_SEND_ENABLED', 'setEnterToSendEnabled', 'enterToSendEnabled', '⏎ Enter to send'],
        ['global-paste', 'GLOBAL_PASTE_ENABLED', 'setGlobalPasteEnabled', 'globalPasteEnabled', '📋 Global paste'],
        ['regular-paste', 'REGULAR_PASTE_ENABLED', 'setRegularPasteEnabled', 'regularPasteEnabled', '📄 Regular paste'],
        ['regular-auto-send', 'REGULAR_AUTO_SEND', 'setRegularAutoSendEnabled', 'regularAutoSend', '➡️ Regular auto-send'],
        ['regular-auto-send-in-input', 'REGULAR_AUTO_SEND_IN_INPUT', 'setRegularAutoSendInInputEnabled', 'regularAutoSendInInput', '⌨️ Auto-send in textbox'],
        ['double-click-edit', 'DOUBLE_CLICK_EDIT_ENABLED', 'setDoubleClickEditEnabled', 'doubleClickEditEnabled', '✏️ Double-click edit'],
        ['nice-auto-paste', 'NICE_AUTO_PASTE_ENABLED', 'setNiceAutoPasteEnabled', 'niceAutoPasteEnabled', '🧲 NICE auto-paste'],
        ['nice-auto-send', 'NICE_AUTO_SEND', 'setNiceAutoSendEnabled', 'niceAutoSend', '🚀 NICE auto-send']
    ];
    ...
    Object.assign(ns.TTSReader, {
        ...
        createUI() {
            ...
            uiPanel.innerHTML = `
                ...
                ${PROMPT_TOGGLES.map(([id, configKey, , , label]) => `<label for="tts-${id}-toggle" style="display:flex; align-items:center; gap:6px; margin-top:6px; cursor:pointer;"><input type="checkbox" id="tts-${id}-toggle" ${this.CONFIG[configKey] ? 'checked' : ''} style="margin:0;">${label}</label>`).join('')}
                ` : ''}
            ...
            if (this.isChatGPTPage) {
                PROMPT_TOGGLES.forEach(([id, configKey, setter, storageKey]) => {
                    const toggle = document.getElementById(`tts-${id}-toggle`);
                    toggle.addEventListener('change', e => {
                        this[setter](e.target.checked);
                        persistProfileSetting(this.settingsProfile, storageKey, this.CONFIG[configKey]);
                    });
        ...
        syncPromptToggles() {
            PROMPT_TOGGLES.forEach(([id, configKey]) => {
                const toggle = document.getElementById(`tts-${id}-toggle`);
                if (toggle) toggle.checked = Boolean(this.CONFIG[configKey]);
            });
        },
